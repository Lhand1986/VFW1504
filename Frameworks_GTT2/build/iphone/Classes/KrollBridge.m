...
NSString * Frameworks_GTT2$ModuleRequireFormat = @"(function(exports){"
...
@implementation Frameworks_GTT2Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_frameworks_gtt2);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Frameworks_GTT2 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Frameworks_GTT2" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_frameworks_gtt2 = [[Frameworks_GTT2Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_frameworks_gtt2];
	
	NSString *_frameworks_gtt2NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _frameworks_gtt2NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_frameworks_gtt2 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Frameworks_GTT2$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.frameworks_gtt2.kroll" 
