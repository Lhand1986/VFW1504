...
//Rare exceptions to indicate a bug in the _frameworks_gtt2 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_frameworks_gtt2 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_frameworks_gtt2]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.frameworks_gtt2.module"
