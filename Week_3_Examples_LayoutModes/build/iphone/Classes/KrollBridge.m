...
NSString * Week_3_Examples_LayoutModes$ModuleRequireFormat = @"(function(exports){"
...
@implementation Week_3_Examples_LayoutModesObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_week_3_examples_layoutmodes);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_week_3_examples_layoutmodes gc];
}
...
{
	// create Week_3_Examples_LayoutModes global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Week_3_Examples_LayoutModes" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_week_3_examples_layoutmodes = [[Week_3_Examples_LayoutModesObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_week_3_examples_layoutmodes];
	
	NSString *_week_3_examples_layoutmodesNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _week_3_examples_layoutmodesNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_week_3_examples_layoutmodes valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Week_3_Examples_LayoutModes$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.week_3_examples_layoutmodes.kroll" 
