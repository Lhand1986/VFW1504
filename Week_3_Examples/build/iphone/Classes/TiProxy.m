...
//Rare exceptions to indicate a bug in the _week_3_examples code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_week_3_examples = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_week_3_examples]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.week_3_examples.module"
