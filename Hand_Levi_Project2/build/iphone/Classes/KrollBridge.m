...
NSString * Hand_Levi_Project2$ModuleRequireFormat = @"(function(exports){"
...
@implementation Hand_Levi_Project2Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_hand_levi_project2);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Hand_Levi_Project2 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Hand_Levi_Project2" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_hand_levi_project2 = [[Hand_Levi_Project2Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_hand_levi_project2];
	
	NSString *_hand_levi_project2NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _hand_levi_project2NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_hand_levi_project2 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Hand_Levi_Project2$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.hand_levi_project2.kroll" 
