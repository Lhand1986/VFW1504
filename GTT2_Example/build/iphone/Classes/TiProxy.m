...
//Rare exceptions to indicate a bug in the _gtt2_example code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_gtt2_example = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_gtt2_example]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.gtt2_example.module"
