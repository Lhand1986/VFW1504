...
NSString * Week_3_Examples_imageLayout$ModuleRequireFormat = @"(function(exports){"
...
@implementation Week_3_Examples_imageLayoutObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_week_3_examples_imagelayout);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_week_3_examples_imagelayout gc];
}
...
{
	// create Week_3_Examples_imageLayout global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Week_3_Examples_imageLayout" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_week_3_examples_imagelayout = [[Week_3_Examples_imageLayoutObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_week_3_examples_imagelayout];
	
	NSString *_week_3_examples_imagelayoutNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _week_3_examples_imagelayoutNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_week_3_examples_imagelayout valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Week_3_Examples_imageLayout$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.week_3_examples_imagelayout.kroll" 
