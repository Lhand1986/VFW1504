...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kWeek_3_Examples_imageLayoutUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kWeek_3_Examples_imageLayoutUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a Week_3_Examples_imageLayout app using the node.js CLI
